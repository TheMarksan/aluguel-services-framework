...
\begin{longtable}{c p{7.5cm} c}
    ...
    2 (Alta)  & API Gateway: roteamento central \textit{stateless} e comunicacao interna via cURL. & Planejado \\
    3 (Alta)  & Microsservico de Autenticacao (\texttt{ms-auth}) com JWT e banco \texttt{db\_auth}. & Planejado \\
    4 (Media) & Microsservico de Catalogo (\texttt{ms-catalogo}) e banco \texttt{db\_catalogo}. & Planejado \\
    5 (Alta)  & Engine de Reservas (\texttt{ms-reservas}): classe abstrata \texttt{RentEngine} (Template Method). & Planejado \\
    6 (Media) & Classes concretas \texttt{LongTermRent} e \texttt{VacationRent}, provando o reuso. & Planejado \\
    7 (Baixa) & Front-end integrado no Gateway (Blade/HTML) consumindo o ecossistema. & Planejado \\
    \bottomrule
...
\begin{lstlisting}[language=PHP,caption={Esboco do Template Method (a ser implementado na Etapa 5).},label={lst:template}]
abstract class RentEngine
{
    // Template Method: esqueleto invariante (frozen spot)
    public final function processReservation(array $request): Reservation
    {
        $this->validateAvailability($request);   // frozen
        $price = $this->calculatePrice($request); // HOTSPOT (abstrato)
        $this->applyBusinessRules($request);       // HOTSPOT (hook)
        return $this->persistReservation($request, $price); // frozen
    }

    abstract protected function calculatePrice(array $request): float; // hotspot

    // Hook: comportamento default sobrescrevivel
    protected function applyBusinessRules(array $request): void {}
}
\end{lstlisting}
