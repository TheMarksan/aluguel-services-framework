...
% ==========================================================================
A camada de apresentacao e servida pelo proprio Gateway (porta 8003) como uma view
HTML/JavaScript, sem expor os microsservicos diretamente ao navegador. O front-end
consome exclusivamente as rotas \texttt{/api/*}, exercitando todo o ecossistema:

\begin{itemize}
    \item \textbf{Autenticacao:} login via \texttt{POST /api/auth/login}; o token JWT
    e armazenado no \texttt{localStorage} e reenviado no cabecalho \texttt{Authorization}
    a cada requisicao, reforcando o carater \textit{stateless} do servidor.
    \item \textbf{Catalogo:} listagem e filtro de imoveis via \texttt{GET /api/imoveis}.
    \item \textbf{Reservas:} criacao via \texttt{POST /api/reservas}, permitindo escolher
    a modalidade (temporada ou longa duracao). Para longa duracao, a interface coleta a
    renda mensal, exibindo o status retornado pela analise de credito (\texttt{confirmed}
    ou \texttt{pending}).
\end{itemize}

Toda a comunicacao trafega pelo Gateway, que reencaminha as chamadas via cURL,
mantendo o front-end desacoplado da topologia interna dos servicos.
